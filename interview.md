...
<details> 
    <summary></summary>
    每条记录使用最后修改事务id、回滚指针和undolog保存了这条记录的版本链，开启一个事务会创建一个读视图（快照）（RC、RR两个隔离级别），读视图中规定了能读取到的记录的版本。
</details>
...
<details> 
    <summary></summary>
    主键索引叶子节点连成链表，若是递增的，那么新插入数据只用插入到链表最后，不用查找插入位置，也减少B+树重新平衡的代价。
</details>
...
<details> 
    <summary></summary>
    淘汰策略，主要是用于内存或缓存满的时候，淘汰最久未使用的数据，来供新数据写入。<br/>
    使用双向链表，每次将访问的节点移到链头，或将新节点插入链头，淘汰的时候淘汰链尾。（或者用LinkedHashMap）
</details>

## 操作系统的内存管理的页面淘汰算法
<details> 
    <summary></summary>
    先进先出；LRU;LFU（淘汰访问次数最少）；时钟算法（先进先出的基础上加访问位），访问位为1的改为0给第二次机会，指针指向下一页面。
</details>
...
<details> 
    <summary></summary>
    wait是Object方法。sleep是Thread方法。<br/>
    空参wait不唤醒就会一直等待。<br/>
    wait会释放对象锁，sleep不会释放锁。
</details>
...
<details> 
    <summary></summary>
    锁(如reentrantlock)的底层框架。通过修改state为1表示获得锁，修改操作由CAS保证原子性，未获得锁的加入等待队列。
</details>
...
<details> 
    <summary></summary>
    若继承两个父类中有同名的方法会发生冲突。Java通过实现多个接口来替代多继承
</details>
...
<details> 
    <summary></summary>
    为了提高redis并发能力，设置redis集群，主节点负责写，从节点负责读。<br/>
    从节点从主节点同步数据：1.从节点向主节点发送replication id和offset，主节点判断是否与自己的replication id一致，若不一致为第一次同步，主节点向从节点发送自己的replication id和offset。2.主节点执行bgsave然后把生成的rdb发送给从节点，在此期间的操作命令会记录在日志文件中一并发送给从节点。3.若不是第一次同步，主节点只会把两者offset偏差的操作命令记录日志发送给从节点。
</details>

## 哨兵机制
<details> 
    <summary></summary>
    哨兵可以监控redis服务器，如果发生故障可以自动恢复<br/>
    哨兵每隔一秒向redis节点发送ping，如果多台（一般为一半）哨兵发现某redis节点无响应则认为该redis节点下线，这时选取一台从节点成为新的主节点（1.从节点的断开时长2.优先级3.offset）。
</details>

## 脑裂问题
<details> 
    <summary></summary>
    哨兵与主节点网络故障，但客户端与主节点正常，这时哨兵选取新的主节点，但客户端仍向旧的主节点写入数据，当哨兵与旧的主节点网络恢复时，旧的主节点会降为从节点导致在此期间客户端写入的数据丢失。<br/>
    解决：1.设置最小从节点数2.设置主从延迟。
</details>

## 分片集群
<details> 
    <summary></summary>
    集群中有多台主节点，每台主节点有多台从节点。<br/>
    主节点之间通过ping互相监测健康状态。<br/>
    读写数据时通过哈希值路由到对应的节点。<br/>
    优点：有多个主节点，所以可以高并发写，存储数据量大；保留了哨兵的功能；保留了高并发读。
</details>
...
<details> 
    <summary></summary>
    内存满的时候，淘汰key<br/>
    1.默认不删除key，只是不允许写入<br/>
    2.删除剩余存活时间最少的<br/>
    下面的面向全部key或者设置了存活时间的key：<br/>
    3.随机选择<br/>
    4.LRU(最近最久未使用算法)<br/>
    5.LFU(最不经常使用算法)
</details>
